...
public class dialogMecanico extends javax.swing.JDialog {
    ...
    private void carregaTable(){
        tableDados.setModel(new MyTableModel(Mecanico.class, dao.read(), tableDados));
    }
    private void carregaTable(String filtro){
        tableDados.setModel(new MyTableModel(Mecanico.class, dao.readByNome(filtro), tableDados));
    }
    ...
    private Mecanico retornaObjeto(){
        Mecanico m = new Mecanico();
        if(textId.getText().isEmpty()){
            m.setIdMecanico(0);
        }else{
            m.setIdMecanico(Integer.parseInt(textId.getText()));
        }
        m.setNome(textNome.getText());
        m.setEspecialidade(textEspecialidade.getText());
        return (m);
    }
    //popula componentes com os valores de um objeto
    private void populaComponentes(Mecanico m){
        textId.setText(m.getIdMecanico()+"");
        textNome.setText(m.getNome());
        textEspecialidade.setText(m.getEspecialidade());
    }
    //validar dados
    ...
    private void tableDadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableDadosMouseClicked
        ...
        if (evt.getClickCount()==2){
            ...
            this.populaComponentes(dao.read(Mecanico.class, codigo));
            jTabbedPane1.setSelectedIndex(0);
        }
    ...
    private void buttonRemoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonRemoverActionPerformed
        ...
        try{
            ...
            dao.delete(dao.read(Mecanico.class, codigo));
            this.iniciaComponentes();
            buttonFiltrar.doClick();
        }catch(Exception ex){
            JOptionPane.showMessageDialog(null, "Erro "+ex.getMessage());
